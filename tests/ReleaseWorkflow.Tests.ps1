...
Assert-True ($CiWorkflow -match 'actions/checkout@v7') 'JourneyDeck CI checkout action must use the Node 24 runtime.'
Assert-True ($CiWorkflow -match 'actions/setup-node@v6') 'JourneyDeck CI setup-node action must use the Node 24 runtime.'
Assert-True ($CiWorkflow -match 'package-manager-cache:\s*false') 'JourneyDeck CI must not enable implicit package-manager caching.'
...
Assert-True (Test-ApprovedDeployPath '.github/workflows/journeydeck-ci.yml' $Rules) 'Approved GitHub workflows must be deployable.'
