...
Assert-True ($Styles -match 'JourneyDeck 5\.9\.3:[\s\S]*?min-height:calc\(44px \+ env\(safe-area-inset-bottom\)\)' -and $Styles -match 'min-height:44px !important') 'Final mobile navigation is still using the legacy two-line height.'
Assert-True ($Styles -match 'JourneyDeck 5\.9\.3:[\s\S]*?#journeyStoryContent\s*\{[^}]*padding-bottom:calc\(76px \+ env\(safe-area-inset-bottom\)\)[^}]*\}[\s\S]*?\.journey-story-actions\s*\{[^}]*position:fixed;[^}]*left:max\(6px,env\(safe-area-inset-left\)\);[^}]*right:max\(6px,env\(safe-area-inset-right\)\);[^}]*bottom:0;[^}]*grid-template-columns:minmax\(0,1fr\) minmax\(0,1fr\)') 'Collection story actions are not side-by-side and anchored to the mobile viewport bottom.'
