...
# Keep all relative fixtures inside the same calendar month, including on the first ten days of a real month.
$FixtureNow=[DateTimeOffset]::Parse('2026-08-16T12:00:00-05:00')
$Drives=@(
    [pscustomobject]@{id='short';startedAt=$FixtureNow.AddDays(-2).ToString('o');miles=32.5;energyKWh=7.15;dateLabel='Monday';startingLocation='Home';endingLocation='Office';efficiencyWhMi=220;soundtrack=@([pscustomobject]@{track='City Lights';artist='Nova Lane';playedAt=$FixtureNow.AddDays(-2).AddMinutes(5).ToString('o')})},
    [pscustomobject]@{id='long';startedAt=$FixtureNow.AddDays(-10).ToString('o');miles=88.2;energyKWh=22.93;dateLabel='Last week';startingLocation='Home';endingLocation='Austin';rawEndingLocation='Downtown Austin';efficiencyWhMi=260;soundtrack=@([pscustomobject]@{track='Open Roads';artist='Nova Lane';playedAt=$FixtureNow.AddDays(-10).AddMinutes(5).ToString('o')})}
)
$UnattachedHistory=@([pscustomobject]@{track='Not While Driving';artist='Decoy';played_at=$FixtureNow.AddMinutes(-5).ToString('o');source='spotify'})

$Longest=Get-DriveOSAssistantAnswer -Question 'What was my longest drive?' -Drives $Drives -Now $FixtureNow
...
$Distance=Get-DriveOSAssistantAnswer -Question 'How many miles did I drive this month?' -Drives $Drives -Now $FixtureNow
...
$Count=Get-DriveOSAssistantAnswer -Question 'How many drives this month?' -Drives $Drives -Now $FixtureNow
...
$Artist=Get-DriveOSAssistantAnswer -Question 'Who is my top artist?' -Drives $Drives -Now $FixtureNow -History $UnattachedHistory
...
$Latest=Get-DriveOSAssistantAnswer -Question 'What was my latest song?' -Drives $Drives -Now $FixtureNow -History $UnattachedHistory
...
$Efficiency=Get-DriveOSAssistantAnswer -Question 'What was my average efficiency this month?' -Drives $Drives -Now $FixtureNow
...
$Destination=Get-DriveOSAssistantAnswer -Question 'How many times have I driven to Austin?' -Drives $Drives -Now $FixtureNow
...
$DriveMusic=Get-DriveOSAssistantAnswer -Question 'What music did I listen to on my drive to Austin?' -Drives $Drives -Now $FixtureNow
...
$Charging=Get-DriveOSAssistantAnswer -Question 'How much did I charge this month?' -Drives $Drives -Now $FixtureNow
...
$Unsupported=Get-DriveOSAssistantAnswer -Question 'Change my charging price' -Drives $Drives -Now $FixtureNow
