...
$required=@('/core/build.js','/core/dom.js','/core/state.js','/core/platform.js','/core/api.js','/components/song-artwork.js','/features/navigation.js','/features/pwa.js','/features/theme.js','/features/ignition.js','/features/places.js','/features/charging.js','/features/recaps.js','/features/refresh.js','/features/drives.js','/features/replay.js','/features/music.js','/features/share-cards.js','/features/command-palette.js','/features/dashboard-customization.js','/app.js')
...
foreach($capability in @('localStorage','dashboard-size-compact','pinned','hidden','draggable')){if($dashboardCustomization -notmatch [regex]::Escape($capability)){throw "Dashboard customization capability is missing: $capability"}}
...
if($styles -notmatch '(?s)\.dashboard-size-wide\.vehicle-panel \.vehicle-car-art\.vehicle-car-model3\.vehicle-car-photo\s*\{[^}]*width:\s*100%\s*!important[^}]*height:\s*100%\s*!important[^}]*object-fit:\s*cover'){throw 'Wide dashboard vehicle artwork framing is missing.'}
