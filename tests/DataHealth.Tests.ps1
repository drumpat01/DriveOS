...
$AdvisoryAuditAlerts = @(Get-DataHealthAlerts -Signals @([pscustomobject]@{ id='integrity-audit'; name='Integrity audit'; status='attention'; lastError='The latest manual parity audit did not pass.' }))
Assert-True ($AdvisoryAuditAlerts.Count -eq 1 -and $AdvisoryAuditAlerts[0].severity -eq 'warning') 'A non-passing manual audit must remain advisory.'
$OverdueAuditAlerts = @(Get-DataHealthAlerts -Signals @([pscustomobject]@{ id='integrity-audit'; name='Integrity audit'; status='stale'; lagMinutes=43201 }) | Where-Object id -eq 'integrity-audit-stale')
Assert-True ($OverdueAuditAlerts.Count -eq 1 -and $OverdueAuditAlerts[0].severity -eq 'warning' -and $OverdueAuditAlerts[0].message -match '30 days') 'An overdue manual audit must create a 30-day advisory warning.'
Assert-True ($Server -match '\$AuditLag -gt 43200' -and $Server -notmatch '\$AuditLag -gt 1560') 'Integrity audits must use the 30-day advisory window.'
