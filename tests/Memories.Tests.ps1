...
Assert-True ($MomentsClient.Contains('const wasSelected = index === state.selectedMemory;') -and $MomentsClient.Contains('if (!wasSelected && !state.records[index]?.create) return;')) 'Off-center Memory cards must center before a second click opens the overview.'
