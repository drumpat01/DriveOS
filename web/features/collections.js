(function(){
  function create({state,api,applyFilter,ensureMapLibre}){
    ...
    async function renderStoryMap(drives){
      ...
      if(!routes.length){mapElement.classList.add("map-unavailable");mapElement.textContent="No route coordinates are available for these drives.";status.textContent="No map data";return;}
      try{
        const maplibre=await ensureMapLibre();
        storyMap=new maplibre.Map({container:mapElement,style:"https://tiles.openfreemap.org/styles/liberty",center:routes[0][0],zoom:9,attributionControl:true});
    ...
    async function openStory(collection){
      ...
      try{
        ...
        $("journeyStoryStats").innerHTML=[{label:"Drives",value:drives.length,detail:"in this collection"},{label:"Distance",value:`${Math.round(miles*10)/10} mi`,detail:"across every route"},{label:"Drive time",value:minutes>=60?`${Math.floor(minutes/60)}h ${minutes%60}m`:`${minutes} min`,detail:"behind the wheel"},{label:"Soundtrack",value:`${music.songs} song${music.songs===1?"":"s"}`,detail:music.artist?`Led by ${music.artist}`:"No archived songs"}].map(item=>`<article class="journey-story-stat"><span>${escape(item.label)}</span><strong>${escape(item.value)}</strong><small>${escape(item.detail)}</small></article>`).join("");
        $("journeyStoryArtist").innerHTML=`${music.image?`<img class="journey-story-artist-art" src="${escape(music.image)}" alt="${escape(music.artist||"Top artist")}">`:'<div class="journey-story-artist-art"></div>'}<span class="section-label">TOP ARTIST</span><strong>${escape(music.artist||"No artist yet")}</strong><small>${music.artist?`${music.plays} play${music.plays===1?"":"s"} in this collection`:"Songs matched to these drives will appear here."}</small>`;
        $("journeyStoryDrives").innerHTML=drives.length?drives.slice(0,12).map(drive=>`<article class="journey-story-drive"><strong>${escape(drive.shortDateLabel||drive.dateLabel||"Drive")}</strong><span>${escape(drive.startingLocation||"Start")} &rarr; ${escape(drive.endingLocation||"Destination")}</span><span>${escape(drive.miles??0)} mi &middot; ${escape(drive.durationMinutes??0)} min</span></article>`).join(""):'<div class="journey-attachment-empty">No drives are currently in this collection.</div>';
    ...
    function render(){
      ...
      container.innerHTML=state.collections.length?state.collections.map(collection=>{const ids=drivesFor(collection),members=membersFor(collection),latest=members[0]?.shortDateLabel||"No drives yet";return `<article class="journey-collection-card" data-open-collection="${escape(collection.id)}" role="button" tabindex="0" aria-label="Open ${escape(collection.name)}"><div><strong>${escape(collection.name)}</strong><p>${escape(collection.description||"A saved group of journeys")}</p><span>${ids.size} drive${ids.size===1?"":"s"} &middot; Latest ${escape(latest)}</span></div><div class="journey-collection-card-actions"><button class="secondary-button" type="button" data-story-collection="${escape(collection.id)}">Open collection</button><button class="text-button" type="button" data-edit-collection="${escape(collection.id)}">Manage</button></div></article>`;}).join(""):'<div class="favorite-routes-empty"><strong>Create your first collection</strong><span>Group existing drives without changing the original JourneyDeck history.</span></div>';
