  ...
  function create({state,api,applyFilter,ensureMapLibre}){
    ...
    async function openStory(collection){
      ...
      try{
        ...
        $("journeyStoryShowDrives").disabled=!drives.length;$("journeyStoryManage").textContent=collection.id?"Edit collection":"Create collection";$("journeyStoryLoading").hidden=true;$("journeyStoryContent").hidden=false;
    ...
    function render(){
      window.DriveOSFeatures.moments?.setCollections(state.collections);
    ...
    async function load(){try{const data=await api.get("/api/collections");state.collections=Array.isArray(data.collections)?data.collections:[];render();}catch(error){const status=$("journeyCollectionsStatus");if(status)status.textContent=error.message;}}
    async function save(event){event.preventDefault();const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;$("journeyCollectionMessage").textContent="Saving…";try{const collection=await api.post("/api/collections/save",{id:$("journeyCollectionId").value||null,name:$("journeyCollectionName").value,description:$("journeyCollectionDescription").value,driveIds:Array.from($("journeyCollectionDriveList").querySelectorAll("input:checked"),input=>input.value)});state.collections=[collection,...state.collections.filter(item=>item.id!==collection.id)];render();document.dispatchEvent(new CustomEvent("journeydeck:collectionchanged",{detail:{collectionId:collection.id}}));close();}catch(error){$("journeyCollectionMessage").textContent=error.message;}finally{submit.disabled=false;}}
    ...
    async function removeAttachment(attachmentId){if(!confirm("Remove this photo or file from the journey?"))return;const message=$("journeyAttachmentMessage"),collectionId=$("journeyCollectionId").value;try{await api.post("/api/collections/attachments/remove",{attachmentId});message.textContent="Removed.";await loadAttachments(collectionId);document.dispatchEvent(new CustomEvent("journeydeck:collectionchanged",{detail:{collectionId}}));}catch(error){message.textContent=error.message;}}
    ...
    $("journeyStoryManage")?.addEventListener("click",()=>{if(!storyCollection)return;const collection=storyCollection;closeStory();if(collection.id){open(collection);return;}open();window.setTimeout(()=>{$("journeyCollectionName").value=collection.name||"";$("journeyCollectionDescription").value=collection.description||"";(collection.driveIds||[]).forEach(driveId=>{const input=document.querySelector(`#journeyCollectionDriveList input[value="${CSS.escape(String(driveId))}"]`);if(input){input.checked=true;input.dispatchEvent(new Event("change",{bubbles:true}));}});},0);});
    document.addEventListener("journeydeck:collectionchanged",event=>{if(event.detail?.source==="moments-create")void load();});
