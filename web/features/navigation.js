(function(){
  ...
  function showView(name){document.querySelectorAll('.view').forEach(view=>view.classList.toggle('active-view',view.id===`view-${name}`));document.querySelectorAll('.nav-button[data-view]').forEach(button=>button.classList.toggle('active',button.dataset.view===name));document.querySelector('[data-mobile-menu]')?.classList.toggle('active',['graph','timeline','music','statistics','health'].includes(name));closeMobileMenu();history.replaceState(null,'',`#${name}`);document.dispatchEvent(new CustomEvent('journeydeck:viewchange',{detail:{view:name}}));window.scrollTo({top:0,behavior:'smooth'});}
