(function(){
  ...
  function create({api,state}){
    ...
    function enrichAddresses(){
      const endpoints=((mapDrives.length?mapDrives:state?.drives)||[]).flatMap(drive=>[
    ...
    async function renderGeographicMap(canvas,status){
      ...
      try{
        ...
        const mapOptions={container:canvas.querySelector('[data-mobility-map]'),style:window.JourneyDeckMapTheme?.style||'https://tiles.openfreemap.org/styles/dark',center:[Number(nodes[0].longitude),Number(nodes[0].latitude)],zoom:10,attributionControl:true};
