(function(){
  ...
  const savedPlaceLabels=()=>{try{return JSON.parse(localStorage.getItem(savedPlaceLabelsStorageKey)||'[]');}catch{return[];}};
  const rememberSavedPlaceLabel=place=>{if(!validCoordinate(Number(place.latitude),Number(place.longitude)))return;const key=`${Number(place.latitude).toFixed(4)},${Number(place.longitude).toFixed(4)}`,places=savedPlaceLabels().filter(item=>`${Number(item.latitude).toFixed(4)},${Number(item.longitude).toFixed(4)}`!==key);places.push({...place,source:'manual',radiusMiles:200/5280,updatedAt:new Date().toISOString()});try{localStorage.setItem(savedPlaceLabelsStorageKey,JSON.stringify(places.slice(-500)));}catch{}};
  ...
  async function loadAtlasPlaceEnrichment(api){
    const withSavedLabels=places=>[...(Array.isArray(places)?places:[]),...savedPlaceLabels()];
    try{
      const result=await api.get('/api/atlas/places');return withSavedLabels(result?.places);
    }catch{return withSavedLabels([]);}
  }
  ...
  function create({api,state}){
    let graph=null,loaded=false,loading=null,selectedId=null,mobilityMap=null,mobilityMarkers=[],placeSaveQueue=Promise.resolve(),routineSaveQueue=Promise.resolve();
    ...
    async function renderGeographicMap(canvas,status){
      ...
      try{
        ...
        map.on('load',()=>{
          const features=representativeJourneyFeatures(state?.drives,200);
    ...
    async function saveCardPlace(card,side,button){const item=(graph?.routines||[]).find(candidate=>candidate.id===card.dataset.routineCard),node=nodeMap().get(item?.[side]),input=card.querySelector(`[data-card-place-name="${side}"]`),status=card.querySelector('[data-card-place-status]'),name=String(input?.value||'').trim();if(!item||!name)return;const category=['home','work','family','errands','dining','wellness','other'].includes(node?.category)?node.category:'other',latitude=Number(item[`${side}Latitude`]??node?.latitude),longitude=Number(item[`${side}Longitude`]??node?.longitude),address=item[`${side}Address`]||node?.address||name,originalText=button.textContent;button.disabled=true;button.textContent='Saving…';if(status){status.textContent='Writing this label…';status.classList.remove('is-error');}const request=async()=>{if(validCoordinate(latitude,longitude)){try{return await api.post('/api/mobility/place-geofence',{latitude,longitude,radiusFeet:200,name,category});}catch(error){if(!/endpoint not found|\b404\b/i.test(String(error?.message||'')))throw error;}}return api.post('/api/mobility/place',{nodeId:item[side],name,category});};placeSaveQueue=placeSaveQueue.catch(()=>{}).then(request);try{await placeSaveQueue;rememberSavedPlaceLabel({latitude,longitude,name,address,category});applySavedCardLabel(item[side],name);loaded=false;button.textContent='Saved';if(status)status.textContent=`${name} saved. You can save the other label now.`;}catch(error){button.textContent=originalText;if(status){status.textContent=error.message||'This label could not be saved.';status.classList.add('is-error');}}finally{button.disabled=false;}}
