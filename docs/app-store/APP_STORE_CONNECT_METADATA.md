# JourneyDeck App Store Connect metadata draft

> Draft for the first public iOS submission. Confirm the legal owner, support contact, URLs, category, and every factual claim before entering it in App Store Connect.

## Required fields

| Field | Draft value | Final check |
| --- | --- | --- |
| App name | `JourneyDeck` | Fits the current App Store name limit. |
| Subtitle | `Your private driving journal` | 29 characters. |
| Primary category | `Lifestyle` | Chosen because JourneyDeck documents a person’s driving life; it does not provide turn-by-turn navigation. |
| Secondary category | `Travel` | Confirm this remains the best fit in App Store Connect. |
| Privacy Policy URL | `https://journeydeck.me/privacy` | Must be a public page, not a login redirect, before entry. |
| Support URL | `https://journeydeck.me/support` | Must include a working, monitored contact method before entry. |
| Copyright | `© 2026 Patrick Benjamin Stewart` | Confirm this is the intended legal copyright owner before submission. |
| Age rating | Complete Apple’s current questionnaire truthfully. | Do not rely on this draft to determine the rating. |

## Promotional text

Your journeys, places, and soundtrack—kept local first.

## Description

JourneyDeck is a private driving journal for the moments that happen on the road.

Record a journey from your iPhone, keep its route and timing in your personal archive, and revisit the places, patterns, memories, and music that make a drive meaningful. JourneyDeck is built local first: recording continues without a server connection, and your saved data remains available on your iPhone.
...
`driving,journal,road trip,route,mileage,trip tracker,car log,memories`

Verify the final value against App Store Connect’s current byte limit and remove duplicates with the title and subtitle if Apple recommends it.

## First-version “What’s New”

Welcome to JourneyDeck — a local-first driving journal for routes, road memories, and optional soundtracks.
