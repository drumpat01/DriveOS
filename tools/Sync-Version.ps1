...
$Expected = @{
    (Join-Path $Root "web\build.json") = "{`n  `"product`": `"DriveOS`",`n  `"version`": `"$Version`",`n  `"webBuild`": `"$WebBuild`",`n  `"features`": [`n    `"friendly-places`",`n    `"charging-history`",`n    `"monthly-recap`",`n    `"lastfm-history`",`n    `"foursquare-business-names`",`n    `"privacy-safe-share-cards`",`n    `"siri-shortcuts`"`n  ],`n  `"css`": `"/styles.css?v=$WebBuild`",`n  `"js`": `"/app.js?v=$WebBuild`"`n}`n"
