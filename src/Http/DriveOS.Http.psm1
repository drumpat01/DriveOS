...
function Get-DriveOSHttpError {
    ...
    elseif($Message -like 'Collection *' -or $Message -like 'A collection *' -or $Message -like 'Attachment *' -or $Message -like 'Attachments *' -or $Message -like 'Memory *' -or $Message -like 'A memory *' -or $Message -like 'One or more memory *' -or $Message -like 'This file type *' -or $Message -like 'A journey *' -or $Message -like '* is required*' -or $Message -like '*Request body was empty*' -or $Message -like '*invalid JSON*'){$result.statusCode=400;$result.statusText='Bad Request';$result.publicMessage=$Message}
