...
try {
  ...
  const legacyPath = config.atlasLegacyDatabasePath ? path.resolve(config.atlasLegacyDatabasePath) : "";
  if (config.atlasDurableTurso && legacyPath && legacyPath !== path.resolve(target) && fs.existsSync(legacyPath)) {
    const legacyDatabase = new DatabaseSync(legacyPath, { readOnly: true });
    try { legacyState = await uploadAtlasDurableState(legacyDatabase); }
    finally { legacyDatabase.close(); }
  }
  const durableState = config.atlasDurableTurso ? await syncAtlasDurableState(database) : null;
  ...
  process.stdout.write(`${JSON.stringify({ synced: true, journeyCount: drives.length, sourceWatermark: built.bootstrap.sourceWatermark, snapshotId: built.snapshotId, legacyState, durableState })}\n`);
} finally {
  database.close();
}
