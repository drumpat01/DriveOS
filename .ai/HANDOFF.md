...
- Implemented but not yet committed or published as an OTA. Release identity is now `P3.3 — Bundle A — music independence`; the installed iPhone remains on P3.1 until an explicit preview OTA publication.
...
- Verification passed: mobile TypeScript, complete mobile suite 82/82, Expo Doctor 21/21, generated Worker types, Worker TypeScript, preview dry-run, live edge smoke tests, and `git diff --check` (Windows line-ending notices only). Remaining physical acceptance after a P3.3 preview OTA: verify Last.fm manual import adds songs with JourneyDeck-server blocking ON; verify Data Health records private-edge/import activity but zero JourneyDeck requests; verify owner Spotify authorization returns through the custom scheme (the redirect must also be registered in the Spotify app dashboard), then sync a recent completed journey. Last.fm public/commercial release remains blocked on written permission.
