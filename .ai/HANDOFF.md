...
- Current state: working tree is intentionally dirty on `agy/journeydeck-1.6`; nothing is committed, pushed, or published to Expo, and no native build was used. The accepted Phase 3.1 changes are ready to commit/push and publish as a runtime 1.7 preview OTA when explicitly requested. Later Phase 3 work remains for direct/stateless provider setup (Spotify/Tessie) and retirement of additional legacy server calls.
