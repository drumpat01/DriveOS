...
- The user's screen recording established that the sheet itself remained open and the welcome content automatically scrolled out of view after its second layout pass, with no additional touch. Commit `48a6b73` disables empty-state auto scrolling while retaining transcript scrolling after a real question/answer. It was published to Build 38's `production` branch/runtime `3.0.0-preview.6`: iOS update `17903659830011`, publish group `494c5216-25ab-433d-84d4-85cb757f904c`, message `Fix_Ask_welcome_auto_scroll`, deployed September 25 at 19:53:04 UTC. Ten focused Ask tests, TypeScript typecheck, publisher export, and diff check pass.
- A corrective OTA fixes the Ask sheet disappearing without user input. The SwiftUI `@expo/ui` text field embedded in the native form sheet was replaced with React Native's stable controlled `TextInput`; transient iOS `inactive` transitions no longer clear the screen, while actual backgrounding and profile changes still discard private answers and pending work.
- Commit `8d06c10` was published through xprem to Build 38's `production` branch/runtime `3.0.0-preview.6`. iOS update `17903656810951`, publish group `0de9f32c-d579-4ea9-93f9-ed903cfb7e11`, message `Build39_Ask_sheet_stability_fix`, deployed September 25 at 19:48:03 UTC. Public manifest returned HTTP 200. Nine focused Ask tests, TypeScript typecheck, publisher iOS export, and `git diff --check` pass. Confirm after up to two cold launches. No native build or Git push was performed.
