...
- Implemented in the dirty working tree on `agy/journeydeck-1.6` at base HEAD `622a44b`; release identity is now `P3.5 — Bundle C — private content`. Nothing is staged, committed, pushed, deployed, published, or built.
...
- Updated the checked-in CloudKit schema additively for Collection/Memory revision and deletion fields plus `Photo` and `PrivatePreference`. Production CloudKit has **not** been updated with this P3.5 schema yet.
...
- Next isolated acceptance order: (1) use live Metro/current 1.7 build with JourneyDeck local-only mode enabled to create/edit/restart a Collection and Memory, add/remove a photo, and delete test content; (2) review and commit this dirty work; (3) import/deploy the additive P3.5 CloudKit schema; (4) include the bridge in the next deliberately consolidated native build; (5) run two-device edit/delete, interrupted-pull replay, photo round-trip, and restore tests before advancing to Phase 4.
