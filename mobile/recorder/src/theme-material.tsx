...
export function ThemeMaterial({ radius = 20 }: { radius?: number }) {
  ...
  return <View pointerEvents="none" accessible={false} style={[StyleSheet.absoluteFill, { borderRadius: radius, overflow: 'hidden' }]}>
    <LinearGradient colors={theme.isLight ? ['#fff8fb55', '#ffffff00', '#d895ab22'] : grandTouring ? [`${theme.palette.green}48`, '#ffffff00', `${theme.palette.accent}10`] : ['#f6f0e218', '#ffffff00', '#d4b15a12']}
      locations={[0, 0.38, 1]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
    <LinearGradient colors={grandTouring ? [`${theme.palette.green}dd`, `${theme.palette.green}22`, `${theme.palette.blue}55`] : [`${theme.palette.chrome}bb`, `${theme.palette.chrome}18`, `${theme.palette.rose}44`]}
