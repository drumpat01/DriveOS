...
function MemoriesScreen({ catalog, journeys, hasMore, loadingMore, onJourney, onRefresh, onLoadMore }: {
  ...
  const { width } = useWindowDimensions();
  const cardWidth = Math.min(270, Math.max(244, width * 0.7));
  const cardGap = 18, cardStep = cardWidth + cardGap, carouselInset = Math.max(18, (width - cardWidth) / 2);
  ...
  const focusMemory = (index: number) => {
    const next = Math.max(0, Math.min(catalog.data.memories.length - 1, index));
    setSelectedIndex(next);
    carousel.current?.scrollTo({ x: next * cardStep, animated: true });
  };
  ...
  return <SafeAreaView style={styles.memoriesSafe}>
    <View pointerEvents="none" style={styles.memoriesAtmosphere}>
      <View style={styles.memoriesAtmosphereWarm} />
      <View style={styles.memoriesAtmospherePurple} />
      <View style={styles.memoriesAtmosphereVignette} />
    </View>
    <ScrollView contentContainerStyle={styles.memoriesPage} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <View style={styles.memoryPageHeader}>
        <View pointerEvents="none" style={styles.memoryHeaderGlow} />
        <View pointerEvents="none" style={styles.memoryHeaderAccent} />
        <Text style={styles.memoryHeaderEyebrow}>THE CHAPTERS YOU KEEP</Text>
        <Text style={styles.memoryHeaderTitle}>MEMORIES</Text>
        <Text style={styles.memoryHeaderBody}>The chapters made from your journeys.</Text>
        <View style={styles.memoryHierarchy}>
          <Text style={styles.memoryHierarchyActive}>MEMORIES</Text><Text style={styles.memoryHierarchyArrow}>→</Text>
          <Text style={styles.memoryHierarchyText}>COLLECTIONS</Text><Text style={styles.memoryHierarchyArrow}>→</Text>
          <Text style={styles.memoryHierarchyText}>JOURNEYS</Text>
        </View>
      </View>
      ...
      <View style={styles.memoryCarouselStage}>
        <Animated.ScrollView
          ref={carousel}
          horizontal showsHorizontalScrollIndicator={false} decelerationRate="fast" snapToInterval={cardStep}
          contentContainerStyle={[styles.memoryCarouselContent, { paddingHorizontal: carouselInset, gap: cardGap }]}
          onScroll={Animated.event([{ nativeEvent: { contentOffset: { x: scrollX } } }], {
            useNativeDriver: true,
            listener: (event: any) => {
              const next = Math.max(0, Math.min(catalog.data.memories.length - 1, Math.round(event.nativeEvent.contentOffset.x / cardStep)));
        ...
        >
          {catalog.data.memories.map((memory, index) => {
            ...
            const rotateY = scrollX.interpolate({ inputRange, outputRange: ['7deg', '0deg', '-7deg'], extrapolate: 'clamp' });
            ...
            return <Animated.View key={memory.id} style={[styles.memoryHeroStage, { width: cardWidth, opacity, transform: [{ perspective: 1100 }, { translateY }, { rotateY }, { scale }] }]}>
              <View style={[styles.memoryHeroGlowFrame, active && styles.memoryHeroGlowFrameActive]}>
                <Pressable onPress={() => { focusMemory(index); setMemoryOverview(memory); }} style={[styles.memoryHeroCard, active && styles.memoryHeroCardActive]}>
                  ...
                  <View pointerEvents="none" style={styles.memoryHeroColorWash} />
                  <View pointerEvents="none" style={styles.memoryHeroShade} />
                  <View pointerEvents="none" style={styles.memoryHeroInnerRim} />
                  <Text style={styles.memoryHeroTitle} numberOfLines={2}>{memory.name}</Text>
                  <Text style={styles.memoryHeroMeta}>{memory.collectionIds.length} collections  •  {journeyIds.size} journeys</Text>
                </Pressable>
              </View>
            </Animated.View>;
          })}
          {!catalog.data.memories.length && <View style={[styles.memoryHeroStage, { width: cardWidth }]}><View style={[styles.memoryHeroGlowFrame, styles.memoryHeroGlowFrameActive]}><Pressable onPress={() => editMemory(null)} style={[styles.memoryHeroCard, styles.memoryHeroCardActive]}><MemoryArtwork artworkKey="road-trips" /><View style={styles.memoryHeroColorWash} /><View style={styles.memoryHeroShade} /><View style={styles.memoryHeroInnerRim} /><Text style={styles.memoryHeroTitle} numberOfLines={2}>Build a chapter</Text><Text style={styles.memoryHeroMeta}>Choose two Collections to begin</Text></Pressable></View></View>}
        </Animated.ScrollView>
        {catalog.data.memories.length > 1 && <>
          <Pressable accessibilityLabel="Previous memory" disabled={selectedIndex === 0} onPress={() => focusMemory(selectedIndex - 1)} style={[styles.memoryCarouselArrow, styles.memoryCarouselPrevious, selectedIndex === 0 && styles.memoryCarouselArrowDisabled]}><Text style={styles.memoryCarouselArrowText}>‹</Text></Pressable>
          <Pressable accessibilityLabel="Next memory" disabled={selectedIndex === catalog.data.memories.length - 1} onPress={() => focusMemory(selectedIndex + 1)} style={[styles.memoryCarouselArrow, styles.memoryCarouselNext, selectedIndex === catalog.data.memories.length - 1 && styles.memoryCarouselArrowDisabled]}><Text style={styles.memoryCarouselArrowText}>›</Text></Pressable>
        </>}
      </View>
      ...
      <View style={styles.memorySectionHeader}><Text style={styles.memoryCollectionHeading} numberOfLines={2}>COLLECTIONS <Text style={styles.memoryCollectionHeadingMuted}>IN {(selectedMemory?.name ?? 'SAVED COLLECTIONS').toUpperCase()}</Text></Text><View style={styles.memoryHeaderActions}>{selectedMemory && <Pressable onPress={() => editMemory(selectedMemory)}><Text style={styles.memoryHeaderAction}>Edit</Text></Pressable>}<Pressable onPress={() => editCollection(null)}><Text style={styles.memoryHeaderAction}>+ New</Text></Pressable></View></View>
...
function MemoryArtwork({ artworkKey, photo }: { artworkKey: string; photo?: JourneyPhoto | null }) {
  ...
  return photo
    ? <View style={styles.memoryArtwork}><View style={styles.memoryPhotoTreatment}><JourneyPhotoImage photo={photo} style={styles.memoryArtworkImage} /></View></View>
...
function CollectionCard({ collection, index, onOpen }: { collection: JourneyCollection; index: number; onOpen: () => void }) {
  ...
  return <Pressable onPress={onOpen} style={({ pressed }) => [styles.memoryCollectionGlowFrame, pressed && styles.memoryCollectionPressed]}>
    <View style={styles.memoryCollectionCard}>
      {collection.photos[0]
        ? <View style={styles.collectionPhotoTreatment}><JourneyPhotoImage photo={collection.photos[0]} style={styles.collectionHeroArtwork} /></View>
        : <View style={[styles.collectionHeroArtwork, styles.collectionFallback, { backgroundColor: `${color}20` }]}><View style={[styles.collectionArtworkOrb, { backgroundColor: color, shadowColor: color }]} /><View style={[styles.collectionArtworkRoute, { backgroundColor: color, shadowColor: color }]} /></View>}
      <View pointerEvents="none" style={[styles.collectionColorWash, { backgroundColor: `${color}13` }]} />
      <View pointerEvents="none" style={styles.collectionShade} />
      <View style={styles.collectionCopy}><Text style={styles.collectionKicker}>COLLECTION</Text><Text style={styles.collectionTitle} numberOfLines={1}>{collection.name}</Text><Text style={styles.collectionMeta} numberOfLines={1}>{collection.driveIds.length} journeys  •  {collection.photos.length} photos{collection.description ? `  •  ${collection.description}` : ''}</Text></View>
      <View style={styles.collectionManage}><Text style={styles.collectionManageText}>›</Text></View>
    </View>
  </Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  memoriesSafe: { flex: 1, backgroundColor: '#07050d' },
  memoriesAtmosphere: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, overflow: 'hidden' },
  memoriesAtmosphereWarm: { position: 'absolute', width: 430, height: 430, borderRadius: 215, left: -125, top: -115, opacity: 0.9, experimental_backgroundImage: 'radial-gradient(circle, rgba(255,82,55,0.16) 0%, rgba(103,27,62,0.07) 42%, transparent 72%)' },
  memoriesAtmospherePurple: { position: 'absolute', width: 510, height: 510, borderRadius: 255, right: -285, top: 350, opacity: 0.85, experimental_backgroundImage: 'radial-gradient(circle, rgba(139,61,219,0.16) 0%, rgba(68,28,111,0.07) 43%, transparent 72%)' },
  memoriesAtmosphereVignette: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, experimental_backgroundImage: 'linear-gradient(135deg, rgba(10,6,18,0.08) 0%, rgba(2,1,7,0.62) 100%)' },
  memoriesPage: { paddingTop: 12, paddingBottom: 128, gap: 16 },
  ...
  memoryPageHeader: { minHeight: 190, marginHorizontal: 12, paddingTop: 20, paddingRight: 18, paddingBottom: 15, paddingLeft: 28, overflow: 'hidden', borderRadius: 22, borderWidth: 1, borderColor: 'rgba(123,72,154,0.26)', experimental_backgroundImage: 'linear-gradient(120deg, rgba(48,17,35,0.88) 0%, rgba(17,8,27,0.94) 48%, rgba(8,5,17,0.98) 100%)', boxShadow: '0 18px 42px rgba(0,0,0,0.34), 0 0 25px rgba(123,54,184,0.12)' },
  memoryHeaderGlow: { position: 'absolute', width: 250, height: 250, borderRadius: 125, right: -82, top: -127, experimental_backgroundImage: 'radial-gradient(circle, rgba(255,89,64,0.23) 0%, rgba(173,56,108,0.08) 45%, transparent 72%)' },
  memoryHeaderAccent: { position: 'absolute', width: 3, left: 10, top: 20, bottom: 57, borderRadius: 2, backgroundColor: '#ff704b', boxShadow: '0 0 14px rgba(255,85,53,0.95)' },
  memoryHeaderEyebrow: { color: '#ff8d6b', fontSize: 9, fontWeight: '900', letterSpacing: 2.25 },
  memoryHeaderTitle: { color: '#fff9f4', fontFamily: 'AvenirNextCondensed-Heavy', fontSize: 46, lineHeight: 49, letterSpacing: 1.2, marginTop: 4, textShadowColor: 'rgba(255,84,54,0.22)', textShadowRadius: 18 },
  memoryHeaderBody: { color: '#aaa0ad', fontSize: 13, lineHeight: 18, marginTop: 1 },
  memoryHierarchy: { minHeight: 37, flexDirection: 'row', alignItems: 'center', gap: 9, marginTop: 14, paddingTop: 12, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(255,105,69,0.34)' },
  memoryHierarchyActive: { color: '#ff795b', fontSize: 9, fontWeight: '900', letterSpacing: 1.15, textShadowColor: 'rgba(255,91,54,0.65)', textShadowRadius: 8 }, memoryHierarchyText: { color: '#a89daf', fontSize: 9, fontWeight: '800', letterSpacing: 1.05 }, memoryHierarchyArrow: { color: '#675d70', fontSize: 14, lineHeight: 16 },
  memorySectionHeader: { marginHorizontal: 20, marginTop: 5, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12 }, memoryLevel: { color: '#ff795b', fontSize: 9, fontWeight: '900', letterSpacing: 1.8, textShadowColor: 'rgba(255,91,54,0.45)', textShadowRadius: 7 }, memoryCollectionHeading: { flex: 1, color: '#f7f0f5', fontSize: 12, lineHeight: 17, fontWeight: '900', letterSpacing: 0.55 }, memoryCollectionHeadingMuted: { color: '#8d8495', fontWeight: '700' }, memorySectionTitle: { color: '#f5f0fb', fontSize: 19, fontWeight: '900', marginTop: 4 }, memoryHeaderActions: { flexDirection: 'row', alignItems: 'center', gap: 12 }, memoryHeaderAction: { color: '#ff8767', fontSize: 11, fontWeight: '900', textShadowColor: 'rgba(255,105,66,0.35)', textShadowRadius: 7 },
  memoryCarouselStage: { height: 420, overflow: 'visible' }, memoryCarouselContent: { paddingTop: 30, paddingBottom: 34 }, memoryHeroStage: { height: 356, justifyContent: 'flex-start' }, memoryHeroGlowFrame: { width: '100%', height: 350, borderRadius: 20, boxShadow: '0 18px 44px rgba(0,0,0,0.5)' }, memoryHeroGlowFrameActive: { boxShadow: '0 22px 56px rgba(0,0,0,0.62), 0 0 28px rgba(255,82,55,0.46)' }, memoryHeroCard: { width: '100%', height: 350, borderRadius: 18, overflow: 'hidden', backgroundColor: '#0a0711', borderWidth: 1, borderColor: 'rgba(187,127,235,0.25)', paddingHorizontal: 18, paddingBottom: 18, justifyContent: 'flex-end' }, memoryHeroCardActive: { borderColor: 'rgba(255,112,75,0.98)' }, memoryHeroColorWash: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, experimental_backgroundImage: 'linear-gradient(145deg, rgba(255,104,71,0.13) 0%, transparent 43%, rgba(117,52,171,0.11) 100%)' }, memoryHeroShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, experimental_backgroundImage: 'linear-gradient(180deg, transparent 34%, rgba(7,4,12,0.22) 54%, rgba(7,4,12,0.97) 100%)' }, memoryHeroInnerRim: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, borderRadius: 17, borderWidth: 1, borderColor: 'rgba(255,157,104,0.32)' }, memoryHeroTitle: { color: '#fffaf7', fontFamily: 'Georgia', fontSize: 27, lineHeight: 30, fontWeight: '400', letterSpacing: -0.35, textShadowColor: 'rgba(0,0,0,0.92)', textShadowRadius: 10 }, memoryHeroMeta: { color: '#c2b7ca', fontSize: 12, fontWeight: '600', marginTop: 7, textShadowColor: 'rgba(0,0,0,0.95)', textShadowRadius: 8 },
  memoryCarouselArrow: { position: 'absolute', zIndex: 20, top: 184, width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(192,139,225,0.34)', backgroundColor: 'rgba(20,14,31,0.9)', boxShadow: '0 10px 24px rgba(0,0,0,0.45), 0 0 14px rgba(159,83,211,0.12)' }, memoryCarouselPrevious: { left: 7 }, memoryCarouselNext: { right: 7 }, memoryCarouselArrowDisabled: { opacity: 0.2 }, memoryCarouselArrowText: { color: '#f4ebf7', fontFamily: 'Georgia', fontSize: 35, lineHeight: 37, marginTop: -3 },
  memoryDots: { minHeight: 10, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10, marginTop: -3 }, memoryDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: 'rgba(162,136,174,0.28)' }, memoryDotActive: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#ff704b', boxShadow: '0 0 11px rgba(255,112,75,0.95)' },
  memoryArtwork: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, backgroundColor: '#241433', overflow: 'hidden' }, memoryArtworkImage: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0 }, memoryPhotoTreatment: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, filter: 'brightness(0.93)', transform: [{ scale: 1.025 }] }, memoryArtworkNight: { backgroundColor: '#0b1630' }, memoryArtworkGlow: { position: 'absolute', width: 260, height: 260, borderRadius: 130, backgroundColor: '#8f3957', opacity: 0.52, left: '50%', marginLeft: -130, top: -75, boxShadow: '0 0 42px rgba(255,113,89,0.72)' }, memoryArtworkMoon: { position: 'absolute', width: 64, height: 64, borderRadius: 32, backgroundColor: '#ff8463', left: '50%', marginLeft: -32, top: 47, boxShadow: '0 0 24px rgba(255,132,99,0.95)' }, memoryArtworkLine: { position: 'absolute', width: 3, height: 235, backgroundColor: '#9d75ff', top: 102, boxShadow: '0 0 10px rgba(168,138,255,0.95)' }, memoryArtworkLineLeft: { left: '50%', marginLeft: -84, transform: [{ rotate: '29deg' }] }, memoryArtworkLineRight: { right: '50%', marginRight: -84, transform: [{ rotate: '-29deg' }] }, memoryArtworkDashOne: { position: 'absolute', width: 3, height: 12, backgroundColor: '#ffd0c4', left: '50%', top: 139 }, memoryArtworkDashTwo: { position: 'absolute', width: 5, height: 25, backgroundColor: '#ff8a68', left: '50%', marginLeft: -1, top: 177 }, memoryArtworkDashThree: { position: 'absolute', width: 7, height: 48, backgroundColor: '#ff795b', left: '50%', marginLeft: -2, top: 242 },
  ...
  memoryCollectionGlowFrame: { height: 142, marginHorizontal: 20, borderRadius: 18, boxShadow: '0 14px 34px rgba(0,0,0,0.36), 0 0 16px rgba(112,56,155,0.09)' }, memoryCollectionPressed: { opacity: 0.76, transform: [{ scale: 0.985 }] }, memoryCollectionCard: { flex: 1, overflow: 'hidden', borderRadius: 17, borderWidth: 1, borderColor: 'rgba(169,104,225,0.28)', backgroundColor: '#0b0712' }, collectionPhotoTreatment: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, filter: 'brightness(0.78)', transform: [{ scale: 1.015 }] }, collectionHeroArtwork: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0 }, collectionFallback: { overflow: 'hidden' }, collectionArtworkOrb: { position: 'absolute', width: 180, height: 180, borderRadius: 90, opacity: 0.38, right: -45, top: -108, shadowOpacity: 0.8, shadowRadius: 22 }, collectionArtworkRoute: { position: 'absolute', width: 250, height: 4, borderRadius: 2, left: -28, top: 80, transform: [{ rotate: '-18deg' }], shadowOpacity: 0.9, shadowRadius: 10 }, collectionColorWash: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0 }, collectionShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, experimental_backgroundImage: 'linear-gradient(180deg, transparent 24%, rgba(7,4,13,0.24) 46%, rgba(7,4,13,0.94) 100%)' }, collectionCopy: { position: 'absolute', left: 16, right: 58, bottom: 14 }, collectionKicker: { color: '#ff8c6b', fontSize: 8, fontWeight: '900', letterSpacing: 1.3, textShadowColor: 'rgba(0,0,0,0.9)', textShadowRadius: 7 }, collectionTitle: { color: '#fff9fc', fontSize: 18, fontWeight: '800', marginTop: 4, textShadowColor: 'rgba(0,0,0,0.95)', textShadowRadius: 9 }, collectionMeta: { color: '#c3b8c8', fontSize: 10, lineHeight: 14, marginTop: 3, textShadowColor: 'rgba(0,0,0,0.95)', textShadowRadius: 8 }, collectionManage: { position: 'absolute', right: 14, bottom: 14, width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(196,156,222,0.38)', backgroundColor: 'rgba(12,8,21,0.78)', boxShadow: '0 8px 20px rgba(0,0,0,0.34)' }, collectionManageText: { color: '#f3e9f7', fontFamily: 'Georgia', fontSize: 29, lineHeight: 31, marginTop: -2 }, managingPill: { color: '#66efc2', fontSize: 8, fontWeight: '900', letterSpacing: 1, borderWidth: 1, borderColor: '#295f4e', borderRadius: 999, paddingHorizontal: 9, paddingVertical: 6 }, journeyManageHelp: { marginHorizontal: 20, color: '#948a9e', fontSize: 11, lineHeight: 17 }, memoryJourneyList: { marginHorizontal: 20, gap: 8 }, journeyMembershipButton: { minHeight: 40, borderRadius: 12, borderWidth: 1, borderColor: '#5d4380', backgroundColor: '#1b1327', alignItems: 'center', justifyContent: 'center' }, journeyMembershipRemove: { borderColor: '#704037', backgroundColor: '#29130f' }, journeyMembershipText: { color: '#c3a5ff', fontSize: 10, fontWeight: '900' }, journeyMembershipRemoveText: { color: '#ff9c80' },
