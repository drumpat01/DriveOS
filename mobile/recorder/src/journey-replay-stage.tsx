...
import { NAVY_FROST } from './navy-frost-policy';
...
export function JourneyReplayStage({ playing, complete, animate, timestamp, song, photo, stop, progress, onToggle, onRestart, onExplore, onPhotoError, onSeek }: {
  ...
  return <NavyFrostSurface reduceTransparency={motion.reduceTransparency} testID="journey-replay-stage" style={s.stage}>
    <View style={s.heading}><Text style={[s.eyebrow, { color: NAVY_FROST.accent }]}>{clock} · {Math.round(progress * 100)}%</Text><TouchPressable accessibilityRole="button" accessibilityLabel="Exit replay and explore map" onPress={onExplore} style={s.close}><Text style={{ color: NAVY_FROST.muted }}>Explore map ↗</Text></TouchPressable></View>
    <Animated.View key={momentKey} entering={animate ? FadeInDown.duration(340).withInitialValues({ opacity: 0, transform: [{ translateY: 16 }] }) : undefined} style={s.moment}>
      {photo && !complete ? <Image source={{ uri: photo.uri }} onError={onPhotoError} accessibilityLabel="Photo from this moment" contentFit="contain" cachePolicy="memory-disk" style={[s.photo, { backgroundColor: NAVY_FROST.solid }]} /> :
        song && !complete && !stop ? <ReplayCover uri={song.artworkUrl} /> : <Animated.View entering={animate ? ZoomIn.duration(260) : undefined} style={[s.badge, { backgroundColor: NAVY_FROST.solid }]}><Text style={{ color: NAVY_FROST.accent, fontSize: 32 }}>{complete ? '✓' : stop ? 'Ⅱ' : '➤'}</Text></Animated.View>}
      <View style={s.copy}><Text style={[s.eyebrow, { color: NAVY_FROST.accent }]}>{arrival}</Text>
        <Text numberOfLines={2} style={[s.title, { color: NAVY_FROST.ink }]}>{complete ? 'You’ve arrived' : stop ? `${Math.max(1, Math.round((stop.end - stop.at) / 60_000))} minute stop` : song?.track ?? 'Watch your journey unfold'}</Text>
        <Text numberOfLines={2} accessibilityLabel={song?.artist} style={[s.detail, { color: NAVY_FROST.muted }]}>{song && !complete ? compactArtistCredit(song.artist) : complete ? 'Every mile, now a memory.' : 'Following your saved route'}</Text>
      </View>
    </Animated.View>
    <TouchPressable onLayout={event => setRailWidth(Math.max(1, event.nativeEvent.layout.width))} onPress={event => onSeek(Math.max(0, Math.min(1, event.nativeEvent.locationX / railWidth)))}
      ...
      <View style={[s.rail, { backgroundColor: `${NAVY_FROST.solid}99` }]}><Animated.View style={{ height: 4, backgroundColor: c.accent, width: `${Math.max(0, Math.min(1, progress)) * 100}%`, transitionProperty: 'width', transitionDuration: animate && playing ? 100 : 0, transitionTimingFunction: 'linear' }} /></View>
    </TouchPressable>
    <View style={s.transport}><TouchPressable accessibilityRole="button" accessibilityLabel="Restart journey story" onPress={onRestart} style={s.restart}><Text style={{ color: NAVY_FROST.ink, fontSize: 23 }}>↺</Text></TouchPressable>
...
function ReplayCover({ uri }: { uri: string | null }) {
  ...
  return source ? <Image source={{ uri: source }} onError={() => setFailed(old => [...old, source])} contentFit="cover" cachePolicy="memory-disk" style={s.cover} /> : <View style={[s.badge, { backgroundColor: NAVY_FROST.solid }]}><Text style={{ color: NAVY_FROST.accent, fontSize: 32 }}>♪</Text></View>;
}
...
const s = StyleSheet.create({
  stage: { borderWidth: 1, borderRadius: 22, padding: 14, gap: 10, borderColor: NAVY_FROST.edge }, heading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 },
