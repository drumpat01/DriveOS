...
1. **Music edit versioning is unresolved:** `musicEntryToCKRecord` sends immutable `createdAt` as `updatedAt`; there is no durable music edit timestamp/revision in the existing local schema. Example: repairing an already-backed-up song's artwork leaves different local/cloud content with the same version. This patch prevents local data loss and reports a conflict, but does not pretend the edit converged. Add explicit music version metadata and conflict handling with the next runtime; introducing schema 7 now would make current schema-6 rollback bundles refuse to open the database.
